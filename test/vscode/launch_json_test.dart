...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_is_github.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggIsGithub = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_is_github.dart',
      );

      expect(
        ggIsGithub,
        isNotNull,
        reason: 'Item for executing bin/gg_is_github.dart not found',
      );

      expect(ggIsGithub['name'], 'gg_is_github.dart');
      expect(ggIsGithub['type'], 'dart');
      expect(ggIsGithub['request'], 'launch');
